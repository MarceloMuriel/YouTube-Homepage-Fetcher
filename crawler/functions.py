...
def readURL(addr, urlvars, headers):
    ...
    except HTTPError as e: print('HTTP Error: {0}. Request: {1}'.format(e.code, "%s/?%s" % (addr, urlencode(urlvars))))
...
def readFeed(full_url, urlvars, headers):
    ...
    if data:
        ...
        feed_nodes = doc.xpath('//ul[@class="feed-list"]/li');
        for feed_node in feed_nodes:
            ...
            #print('feed:', feed)
            yield feed
        print('video exploration completed', full_url)
    else:
